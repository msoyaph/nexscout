...
function PostEditor({ post, companies, onClose }: { post?: any; companies: any[]; onClose: () => void }) {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      // Step 1: Create or update admin_companies first
      const companyData = {
        name: formData.company_name,
        industry: formData.category || 'Other',
        short_description: formData.excerpt || '',
        logo_url: formData.company_logo_url || null,
        is_active: true,
        is_featured: false,
        owner_type: 'system',
      };
      ...
      if (companyError) {
        ...
        alert('Error saving company: ' + companyError.message);
        setSaving(false);
        return;
      }
      ...
      // Step 2: Save AI instructions to company_knowledge_posts
      const dataToSave: any = {
        title: formData.company_name + ' AI Instructions',
        content: formData.content,
        excerpt: formData.excerpt || '',
        post_type: 'ai_system_instructions',
        category: formData.category || '',
        tags: formData.tags ? formData.tags.split(',').map(t => t.trim()).filter(Boolean) : [],
        status: formData.status || 'draft',
        admin_company_id: companyResult.id,
      ...
      if (products.filter(p => p.name).length > 0) {
        ...
        if (productsError) {
          console.warn('Products save warning:', productsError);
          // Don't block - products are optional
        } else {
          console.log('Products saved:', productInserts.length);
        }
      }

      console.log('Template saved successfully!');
      alert(`✅ Template Saved Successfully!\n\nCompany: ${formData.company_name}\nAI Instructions: ${formData.content.length} characters\nProducts: ${products.filter(p => p.name).length}\nStatus: ${formData.status || 'draft'}\n\nUsers can now select this template during onboarding!`);
      setSaving(false);
      onClose();
    } catch (error: any) {
