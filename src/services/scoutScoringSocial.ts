/**
 * Social Graph Scoring Engine
 * Moved from v3 to separate module for clarity
 */
import { supabase } from '../lib/supabase';
import { PainPointProfile } from './painPointClassifierPH';
import { SocialGraphBuilder } from './socialGraphBuilder';

export interface SocialProspectSignals {
...
export class ScoutSocialScoreEngine {
  ...
  static calculateSocialScore(signals: SocialProspectSignals): SocialScoreResult {
  ...
  static async scoreProspectsForScanSocial(scanId: string): Promise<void> {
    ...
    for (const [name, prospect] of prospectMap.entries()) {
      ...
      const signals: SocialProspectSignals = {
      ...
      const scoreResult = this.calculateSocialScore(signals);
