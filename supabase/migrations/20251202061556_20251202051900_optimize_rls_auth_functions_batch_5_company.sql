/*
  # Optimize RLS Auth Functions - Batch 5 (Company Tables)

  1. Performance Optimization
    - Replace auth.uid() with (select auth.uid())
  
  2. Affected Tables (Batch 5)
    - company_profiles
    - company_crawl_sessions
    - company_embeddings
    - company_intelligence
    - company_knowledge_posts
...
BEGIN
  FOR policy_record IN 
    ...
    WHERE schemaname = 'public'
      AND tablename IN ('company_profiles', 'company_crawl_sessions', 'company_embeddings', 
                        'company_intelligence', 'company_knowledge_posts', 'company_multi_site_data')
