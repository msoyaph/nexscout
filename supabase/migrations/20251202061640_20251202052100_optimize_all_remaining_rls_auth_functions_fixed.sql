/*
  # Optimize All Remaining RLS Auth Functions

  1. Performance Optimization
    - Replace auth.uid() with (select auth.uid()) across ALL remaining tables
    - Dramatically improves RLS policy execution performance
  
  2. Coverage
    - Processes all tables in public schema
  ...
  LOOP
    ...
    ELSIF policy_record.cmd = 'INSERT' THEN
      EXECUTE format('CREATE POLICY %I ON %I FOR INSERT TO authenticated WITH CHECK (%s)',
        policy_record.policyname, policy_record.tablename, COALESCE(new_with_check, 'true'));
    ELSIF policy_record.cmd = 'UPDATE' THEN
      ...
      ELSIF new_qual IS NOT NULL THEN
        EXECUTE format('CREATE POLICY %I ON %I FOR UPDATE TO authenticated USING (%s)',
          policy_record.policyname, policy_record.tablename, new_qual);
      ELSIF new_with_check IS NOT NULL THEN
        EXECUTE format('CREATE POLICY %I ON %I FOR UPDATE TO authenticated WITH CHECK (%s)',
