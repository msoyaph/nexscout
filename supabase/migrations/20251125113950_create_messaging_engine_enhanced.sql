/*
  ...
  1. New Tables
    - `message_sequences` (parent for sequences)
    - `prospects` (parent for messaging + scoring)
    - `prospect_scores` (base scoring table)
...
-- ============================================================================
-- 0A. MESSAGE SEQUENCES (Parent table for sequence_steps)
-- ============================================================================
...
-- ============================================================================
-- 0B. PROSPECTS TABLE (Parent table for generated_messages + scores)
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS prospects (
  ...
  status text DEFAULT 'new'
    CHECK (status IN ('new', 'contacted', 'interested', 'follow_up', 'closed', 'lost')),
  tags text[],
...
-- ============================================================================
-- 0C. PROSPECT_SCORES TABLE (Parent table required before enhancements)
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS prospect_scores (
  ...
  score_category text DEFAULT 'unknown'
    CHECK (score_category IN ('hot', 'warm', 'cold', 'unknown')),
  created_at timestamptz DEFAULT now(),
  updated_at timestamptz DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS generated_messages (
  ...
  intent text
    CHECK (intent IN ('recruit', 'sell', 'follow_up', 'reconnect', 'introduce', 'book_call')),
  tone text
    CHECK (tone IN ('professional', 'friendly', 'casual', 'direct')),
...
CREATE TABLE IF NOT EXISTS user_library (
  ...
  content_type text NOT NULL
    CHECK (content_type IN ('message', 'sequence', 'deck', 'template', 'snippet')),
  title text NOT NULL,
...
CREATE TABLE IF NOT EXISTS ai_usage_limits (
  ...
  usage_type text NOT NULL
    CHECK (usage_type IN ('message', 'sequence', 'deck', 'deepscan')),
  usage_period text NOT NULL
    CHECK (usage_period IN ('daily', 'weekly', 'monthly')),
...
--    (Assumes message_sequences, pitch_decks, ai_message_sequences already exist)
-- ============================================================================

-- Add prospect-related fields to message_sequences
DO $$
BEGIN
  IF NOT EXISTS (
  ...
  IF NOT EXISTS (
    ...
    ALTER TABLE message_sequences
      ADD COLUMN sequence_type text DEFAULT 'follow_up'
        CHECK (sequence_type IN ('cold_outreach', 'follow_up', 'nurture', 'reconnect', 'close'));
  END IF;
  ...
  IF NOT EXISTS (
    ...
    ALTER TABLE message_sequences
      ADD COLUMN total_steps integer DEFAULT 5;
  END IF;
...
-- Add prospect linkage + metadata to pitch_decks (if table exists)
DO $$
BEGIN
  IF EXISTS (
    ...
    IF NOT EXISTS (
      ...
      ALTER TABLE pitch_decks
        ADD COLUMN version text DEFAULT 'basic'
          CHECK (version IN ('basic', 'elite'));
    END IF;
...
-- Add prospect_id to ai_message_sequences (if table exists)
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.tables
    WHERE table_name = 'ai_message_sequences'
  ) THEN
    IF NOT EXISTS (
      SELECT 1 FROM information_schema.columns 
      WHERE table_name = 'ai_message_sequences' AND column_name = 'prospect_id'
    ) THEN
      ALTER TABLE ai_message_sequences
        ADD COLUMN prospect_id uuid REFERENCES prospects(id) ON DELETE CASCADE;
    END IF;
  END IF;
END $$;
...
CREATE INDEX IF NOT EXISTS idx_sequence_steps_status
  ON sequence_steps(status);

CREATE INDEX IF NOT EXISTS idx_sequence_steps_send_date
  ON sequence_steps(recommended_send_date);

CREATE INDEX IF NOT EXISTS idx_generated_messages_user_id
  ON generated_messages(user_id);

CREATE INDEX IF NOT EXISTS idx_generated_messages_prospect_id
  ON generated_messages(prospect_id);

CREATE INDEX IF NOT EXISTS idx_generated_messages_created_at
  ON generated_messages(created_at DESC);

CREATE INDEX IF NOT EXISTS idx_user_library_user_id
  ON user_library(user_id);

CREATE INDEX IF NOT EXISTS idx_user_library_content_type
  ON user_library(content_type);

CREATE INDEX IF NOT EXISTS idx_user_library_is_favorite
  ON user_library(is_favorite);

CREATE INDEX IF NOT EXISTS idx_user_library_created_at
  ON user_library(created_at DESC);

CREATE INDEX IF NOT EXISTS idx_ai_usage_limits_user_id
  ON ai_usage_limits(user_id);

CREATE INDEX IF NOT EXISTS idx_ai_usage_limits_period
  ON ai_usage_limits(usage_type, usage_period, period_start);
...
-- Function to check if user has reached daily/weekly/monthly limit
...
BEGIN
  ...
  -- Get usage record
  SELECT usage_count, limit_amount
